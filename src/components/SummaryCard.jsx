// Named exports
export function SummaryCard({ title, value, subtext, icon, active = false, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <div className="flex items-center gap-3">
        <div className={[
          ...
          active ? "bg-blue-50 text-blue-600" : "bg-gray-50 text-gray-700"
        ].join(" ")}>
          {icon}
        </div>
        <div>
          ...
          {subtext ? <p className="text-xs text-gray-500 mt-0.5">{subtext}</p> : null}
        </div>
      </div>
    </button>
  );
